...
impl VM {
            ...
            if let Ok(line) = line {
                ...
                if !parts.is_empty() {
                    match parts[0] {
                        ...
                        "ret" => self.program.push(Instruction::Ret),
                        "stop" => self.program.push(Instruction::Stop),
                        ...
                        "label" => {
                            if let Ok(value) = parts[1].parse::<String>() {
                                ...
                                self.labels.insert(value, self.program.len());
                            } else {
                                panic!("Error parsing value on line");
                            }
                        }
    ...
    fn cycle(&mut self) {
        let mut skip = false;
        if skip {
            while Instruction::Stop == self.program[self.pc] {
                self.pc += 1;
            }
            skip = false;
        }
